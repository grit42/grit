...
import { classnames, generateUniqueID } from "@grit42/client-library/utils";
import DeleteIcon from "@grit42/client-library/icons/Delete";
import CloneIcon from "@grit42/client-library/icons/Clone";
import { Checkbox, Tooltip } from "@grit42/client-library/components";
...
/**
 * The `useReactTable` setup, column/action wiring, row-virtualization
 * container tracking, and column-size CSS vars shared verbatim by `Table`
 * and `DataGrid` — the only thing that differs between those two components
 * is the outer JSX shell (title/filters/settings layout) built around this.
 */
export default function useTableCore<T>(
  ...
  const { data, onSelect, getRowId } = props;
  ...
  const columnsWithActions = useMemo(() => {
    ...
    const columnsWithNormalizedFlex = columns.map((column) => {
      if (!("flex" in column) || !column.flex) return column;
      if (import.meta.env.DEV && column.size !== undefined) {
        console.warn(
          `Column "${column.id}" sets both "size" and "flex" — "flex" takes precedence over the fixed pixel width, though "size" still applies as a minimum width.`,
        );
      }
      return { ...column, enableResizing: false };
    });
    if (!columnsBefore.length && !columnsAfter.length) {
      return columnsWithNormalizedFlex;
    }
    return [...columnsBefore, ...columnsWithNormalizedFlex, ...columnsAfter];
  }, [columns, props, setSelectAllState, settings?.enableSelection, styles]);
  ...
  // NOTE: these vars hold each column's TanStack `getSize()` (its declared
  // `size`/`minSize`, unrelated to `flex`). Flex columns' *rendered* width is
  // determined by the browser's flexbox layout, not this value — a future
  // frozen-columns feature that sums these to compute a sticky `left` offset
  // will need to special-case flex columns rather than relying on this var.
  ...
  return {
    ...
    displayData: data ?? (FALLBACK_DATA as T[]),
  };
}

export { getCellSizeStyle };
