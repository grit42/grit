    ...
    def self.detailed(params = nil)
      ...
      BatchProperty.all.each do |property|
        ...
        if property.data_type.is_entity
          ...
          query = query
            .joins("LEFT OUTER JOIN #{property.data_type.table_name} dt__#{property.data_type.id}__#{property.safe_name} on dt__#{property.data_type.id}__#{property.safe_name}.id = bpv__#{property.safe_name}.entity_id_value")
