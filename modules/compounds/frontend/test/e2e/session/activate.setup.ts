...
setup("activate admin user", async ({ page }) => {
  ...
  // Wait for either: redirect to compounds (user is activated) OR "is inactive" message (user needs activation)
  const inactiveMessage = page.getByText("is inactive");
  const compounds = "http://localhost:3001/app/compounds";

  // Use Promise.race to detect which scenario we're in
  const result = await Promise.race([
    page.waitForURL(compounds).then(() => "activated"),
  ...
  // Verify we ended up on the compounds page
