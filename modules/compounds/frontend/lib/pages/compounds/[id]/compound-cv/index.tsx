...
const CompoundDetails = () => {
  ...
  const OrderGeneralInfo = ['Number', 'Name', 'Molecular formula', 'inchi', 'inchi key'];
  const OrderGeneralInfoLC = OrderGeneralInfo.map(s => s.toLowerCase());
  const AllowedGeneralInfo = new Set(OrderGeneralInfoLC);


  const OrderCalculatedProperties = ['MW', 'logP', 'Hydrogen Bond Donor Count', 'Hydrogen Bond Acceptor Count'];
  const OrderCalculatedPropertiesLC = OrderCalculatedProperties.map(s => s.toLowerCase());
  ...
  const generalInfoItems = useMemo(() => filteredFields(AllowedGeneralInfo, OrderGeneralInfoLC), [compound, compoundTypeFields]);
  const CalculatedPropertiesInfoItems = useMemo(() => filteredFields(AllowedCalculatedProperties, OrderCalculatedPropertiesLC), [compound, compoundTypeFields]);
  ...
  return (
    ...
    <div className={styles.container}>
      ...
      <button onClick={CharacterizationToggle} aria-expanded={showCharacterization}>
        Characterization
      </button>
