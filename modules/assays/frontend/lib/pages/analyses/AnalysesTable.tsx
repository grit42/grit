...
const AnalysesTable = () => {
  ...
  const canManageAnalysis = useHasPermission("write:analysis");
  const { pathname } = useLocation();
  const { data: analysisColumns } = useAnalysisColumns(undefined, {
    select: (data) =>
      data.map((d) => ({
        ...d,
        default_hidden:
          d.default_hidden || d.name === "assay_data_sheet_definition_id__name",
      })),
  });

  const analysisTableColumns = useTableColumns(analysisColumns);
  ...
  const { data, isLoading, isFetchingNextPage, isError, error, fetchNextPage } =
    useInfiniteAnalyses(tableState.sorting, tableState.filters);
  ...
  }, [registerToolbarActions, navigateToNew, pathname, canManageAnalysis]);
