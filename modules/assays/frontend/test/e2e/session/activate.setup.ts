...
setup("activate admin user", async ({ page }) => {
  ...
  // Wait for either: redirect to experiments (user is activated) OR "is inactive" message (user needs activation)
  const inactiveMessage = page.getByText("is inactive");
  const experimentsUrl = "http://localhost:3001/app/assays/experiments";

  // Use Promise.race to detect which scenario we're in
  const result = await Promise.race([
    page.waitForURL(experimentsUrl).then(() => "activated"),
  ...
  if (result === "inactive") {
    ...
    await page.waitForURL(experimentsUrl);
  }

  // Verify we ended up on the experiments page
  await expect(page).toHaveURL(experimentsUrl);
});
