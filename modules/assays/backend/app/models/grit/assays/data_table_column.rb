      ...
      when "string","text"
        ...
        when "count"
          return subquery.select(*[
            ...
            assay_data_sheet_column.data_type.model.display_properties.map do |display_property|
              "count(dt__#{assay_data_sheet_column.safe_name}.#{display_property[:name]}) AS value__#{display_property[:name]}"
            end
          ])
        when "csv"
          return subquery.select(*[
            ...
            assay_data_sheet_column.data_type.model.display_properties.map do |display_property|
              "STRING_AGG(dt__#{assay_data_sheet_column.safe_name}.#{display_property[:name]}, ', ') AS value__#{display_property[:name]}"
            end
          ])
        end
      end
...
JOIN GRIT_ASSAYS_EXPERIMENTS ON GRIT_ASSAYS_EXPERIMENTS.ID = GRIT_ASSAYS_EXPERIMENT_DATA_SHEETS.EXPERIMENT_ID
    ...
    def join_entity_table subquery
      if assay_data_sheet_column.data_type.is_entity
        entity_join = <<-SQL
LEFT OUTER JOIN #{assay_data_sheet_column.data_type.table_name} dt__#{assay_data_sheet_column.safe_name} ON
dt__#{assay_data_sheet_column.safe_name}.id = data_sources.entity_id_value
        SQL
