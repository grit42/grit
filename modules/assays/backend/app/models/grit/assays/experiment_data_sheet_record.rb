      ...
      def self.for_sheets(assay_data_sheet_definition_id, sheet_ids)
        ...
        Grit::Assays::AssayDataSheetColumn.where(assay_data_sheet_definition_id: assay_data_sheet_definition_id).all.each do |column|
          ...
          if column.data_type.is_entity
            ...
            query = query
              .joins("LEFT OUTER JOIN #{column.data_type.table_name} cpv__#{column.safe_name} on cpv__#{column.safe_name}.id = ev__#{column.safe_name}.entity_id_value")
            ...
            for display_property in entity_klass.display_properties do
              query = query
                .select("cpv__#{column.safe_name}.#{display_property[:name]} as #{column.safe_name}__#{display_property[:name]}") unless entity_klass.display_properties.nil?
            end
          else
