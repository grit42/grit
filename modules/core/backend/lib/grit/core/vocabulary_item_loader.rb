...
module Grit::Core
  class VocabularyItemLoader < Grit::Core::EntityLoader
    ...
    def self.fields(params)
      [ *super(params), *Grit::Core::VocabularyItemLoadSet.entity_fields ]
    end
    ...
    def self.create(params)
      ...
      vocabulary_item_load_set = Grit::Core::VocabularyItemLoadSet.new({
        load_set_id: load_set.id,
        vocabulary_id: params[:vocabulary_id]
      })
      vocabulary_item_load_set.save!

      load_set
    end

    def self.show(load_set)
      vocabulary_item_load_set = Grit::Core::VocabularyItemLoadSet.find_by(load_set_id: load_set.id)
      return load_set.as_json if vocabulary_item_load_set.nil?
      { **vocabulary_item_load_set.as_json, **load_set.as_json }
    end

    def self.destroy(load_set)
      Grit::Core::VocabularyItemLoadSet.destroy_by(load_set_id: load_set.id)
      super
    end

    def self.validate(load_set)
      vocabulary_item_load_set = Grit::Core::VocabularyItemLoadSet.find_by(load_set_id: load_set.id)
      load_set_entity_properties = Grit::Core::VocabularyItem.entity_properties(vocabulary_id: vocabulary_item_load_set.vocabulary_id)
      ...
      data.each_with_index do |datum, index|
        record_props = {}
        record_props["vocabulary_id"] = vocabulary_item_load_set.vocabulary_id
        has_errors = false
        ...
        ActiveRecord::Base.transaction(requires_new: true) do
          ...
          duplicate_names = names.select { |o| o == name }

          names.push(name)

          if duplicate_names.length.positive?
            record_errors["name"] = [ "should be unique (duplicate in file)" ]
          end

          if record_errors.empty?
            vocabulary_item = Grit::Core::VocabularyItem.new({
              name: record_props["name"],
              description: record_props["description"],
              vocabulary_id: record_props["vocabulary_id"]
            })

            unless vocabulary_item.valid?
              errors.push({ index: index, datum: datum, errors: vocabulary_item.errors })
              has_errors = true
            end
          else
    ...
    def self.confirm(load_set)
      ...
      ActiveRecord::Base.transaction do
        Grit::Core::LoadSetLoadingRecord.includes(:load_set_loading_record_property_values).where(load_set_id: load_set.id).each do |loading_record|
          ...
          record = Grit::Core::VocabularyItem.create!(record_props)

          Grit::Core::LoadSetLoadedRecord.new({
            load_set_id: load_set.id,
            table: "grit_core_vocabulary_items",
            record_id: record.id
          }).save!
        end
    ...
    def self.mapping_fields(load_set)
      vocabulary_item_load_set = Grit::Core::VocabularyItemLoadSet.find_by(load_set_id: load_set.id)
      Grit::Core::VocabularyItem.entity_fields(vocabulary_id: vocabulary_item_load_set.vocabulary_id).filter { |f| ![ "vocabulary_id" ].include?(f[:name]) }
    end
  end
end
