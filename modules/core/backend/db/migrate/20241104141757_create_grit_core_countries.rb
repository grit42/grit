class CreateGritCoreCountries < ActiveRecord::Migration[7.2]
  def up
    create_table :grit_core_countries, id: false do |t|
      ...
      t.string :name, null: false, index: { unique: true, name: 'idx_countries_on_name_unique' }
      t.string :iso, null: false, index: { unique: true, name: 'idx_countries_on_iso_unique' }
    end
