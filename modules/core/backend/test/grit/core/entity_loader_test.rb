...
class Grit::Core::EntityLoaderTest < ActiveSupport::TestCase
  ...
  test "loader resolves a specialized loader class by entity name" do
    assert_equal Grit::TestEntityLoader, Grit::Core::EntityLoader.loader("Grit::TestEntity")
  end
  ...
  test "records_from_csv yields correct field count for TSV with empty trailing columns" do
    data_proxy = Object.new
    fixture = file_fixture("test_entity_empty_columns.tsv")
    data_proxy.define_singleton_method(:open) { |&block| fixture.open(&block) }
    lsb = OpenStruct.new(data: data_proxy, separator: "\t")

    rows = []
    Grit::Core::EntityLoader.send(:records_from_csv, lsb) { |row| rows << row }

    assert_equal 10, rows.size
    rows.each do |row|
      fields = CSV.parse_line(row)
      assert_equal 5, fields.size, "Expected 5 fields (line + 4 columns), got: #{fields.inspect}"
    end
  end
