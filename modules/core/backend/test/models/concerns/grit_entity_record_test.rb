...
class GritEntityRecordTest < ActiveSupport::TestCase
  ...
  test "length validation mechanism is applied for string columns with limits" do
    ...
    assert_empty Grit::TestEntity._validators[:created_by].to_a
    assert_empty Grit::TestEntity._validators[:updated_by].to_a
  end
  ...
  test "foreign_keys returns foreign key constraints" do
    ...
    # Grit::TestEntity has a user_id foreign key
  ...
  test "detailed scope joins foreign key tables" do
    ...
    entity = Grit::TestEntity.create!(name: "With User", user_id: grit_core_users(:admin).id)
  ...
  test "loader_find_by! finds record by property" do
    entity = grit_test_entities(:one)
    found = Grit::TestEntity.loader_find_by!(:name, "one")
