...
import LoadSetDataUpdateDialog from "./LoadSetDataUpdateDialog";
        ...
        <div className={styles.header}>
          ...
          <form.Subscribe
            ...
            children={([canSubmit, isSubmitting]) => (
              <ButtonGroup>
                <Button onClick={handleCancel} loading={rollbackLoadSetMutation.isPending || destroyLoadSetMutation.isPending}>
                  {isValidated ? "Make changes" : "Cancel import"}
                </Button>
                {!isValidated && <Button onClick={() => setIsOpen(true)}>Edit data set</Button>}
                {loadSet.status_id__name === "Invalidated" && (
                  <Button loading={confirmLoadSetMutation.isPending} color="danger" onClick={handleConfirm}>
...
const MappingLoadSetWrapper = ({ id }: { id: string | number }) => {
  const {
    data: loadSet,
    isLoading: isLoadSetLoading,
    isError: isLoadSetError,
    error: loadSetError,
  } = useEntityDatum<LoadSetData>("grit/core/load_sets", id.toString());
  ...
  const {
    ...
    isError: isPreviewDataError,
    error: previewDataError,
  } = useLoadSetPreviewData(Number(id));
  ...
  const {
    ...
    isError: isFieldsError,
    error: fieldsError,
  } = useLoadSetMappingFields(Number(id));
  ...
  if (isLoadSetLoading || isFieldsLoading || isPreviewDataLoading) {
    return <Spinner />;
  }
  ...
  if (
    ...
    return (
      <ErrorPage error={loadSetError ?? fieldsError ?? previewDataError} />
    );
  }
